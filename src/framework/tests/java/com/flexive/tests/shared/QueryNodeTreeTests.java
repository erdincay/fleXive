...
public class QueryNodeTreeTests {
    private static FxPropertyAssignment createAssignment() {
        FxProperty property = new FxProperty(42, "testProperty", new FxMultiplicity(0, 1), null, FxDataType.String1024);
        return new FxPropertyAssignment(
                43, true, null, null, "test", 0,
                property.getMultiplicity(), 0, null, 0, null, null, null, property, null, FxLanguage.SYSTEM_ID, null);
    }
...
//						.substring(0, 256))).isValid() : "256 character string should'nt be valid";
    ...
    public void selectListNodeTest() {
        ...
        QueryRootNode root = new QueryRootNode(0, QueryRootNode.Type.CONTENTSEARCH);
        final FxPropertyAssignment assignment = createAssignment();
    ...
    public void propertyValueNodeTest() {
        ...
        root.addChild(child2);
        assert root.isValid();
        assert root.getSqlQuery().contains("co." + property.getName() + " = 1 AND co." + property.getName() + " != 2");
    }
