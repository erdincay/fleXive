...
CREATE TABLE FXS_ACCOUNTS (
 ...
 INDEX IDX_ACCOUNTS_REST_TOKEN(REST_TOKEN),
 UNIQUE UK_ACCOUNTS_LOGIN_NAME(USERNAME),
 UNIQUE UK_ACCOUNTS_REST_TOKEN(REST_TOKEN),
...
-- PRIMARY KEY(ID,APPLICATION), -- don't use primary key to avoid locking tables for logins (FX-868)
 INDEX IDX_ACCOUNT_DETAILS_ID_APP(ID, APPLICATION),
...
CREATE INDEX FXI_HISTORY ON FXS_HISTORY(MANDATOR,APPLICATION,LOGINNAME,TIMESTP,TYPENAME);
...
CREATE TABLE FXS_TYPEDEF (
  ...
  AUTO_VERSION BOOLEAN NOT NULL DEFAULT FALSE COMMENT 'When true and contents have changed, a new version will be created when saving.',
...
ENGINE=InnoDB
DEFAULT CHARSET = LATIN1
COMMENT='Type Groups translations'
ROW_FORMAT=FIXED;
...
CREATE TABLE FXS_DATATYPES (
  ...
  UNIQUE KEY UK_DATATYPES_NAME(NAME)
)
ENGINE=InnoDB
DEFAULT CHARSET = LATIN1
ROW_FORMAT=FIXED
...
CREATE TABLE FX_BINARY (
  ...
  PREV4_WIDTH INTEGER UNSIGNED,
  PREV4_HEIGHT INTEGER UNSIGNED,
  PREV1SIZE INTEGER UNSIGNED,
  PREV2SIZE INTEGER UNSIGNED,
  PREV3SIZE INTEGER UNSIGNED,
  PREV4SIZE INTEGER UNSIGNED,
...
ENGINE=MyISAM
COMMENT='Binary transit (temporary binary storage)'
DEFAULT CHARSET=LATIN1;
...
CREATE TABLE FXS_SELECTLIST (
  ...
  BCSEP VARCHAR(255) CHARACTER SET UTF8 NOT NULL DEFAULT ' > ',
  SAMELVLSELECT BOOLEAN DEFAULT FALSE,
  SORTENTRIES BOOLEAN DEFAULT FALSE,
  PRIMARY KEY (ID),
  UNIQUE KEY UK_SELECTLIST_NAME(NAME),
...
ENGINE = InnoDB
DEFAULT CHARSET = LATIN1
COMMENT = 'Selectlist Items translations';
...
CREATE TABLE FXS_TYPEPROPS (
  ...
  DEFAULT_VALUE LONGTEXT CHARACTER SET UTF8 COMMENT 'Default value serialized to XML',
  PRIMARY KEY (ID),
...
CREATE TABLE FXS_GROUP_OPT (
  ...
  FOREIGN KEY FK_GROUP_ID(ID) REFERENCES FXS_TYPEGROUPS(ID)
  	ON DELETE RESTRICT ON UPDATE RESTRICT,
  FOREIGN KEY FK_GROUP_ASSID(ASSID) REFERENCES FXS_ASSIGNMENTS(ID)
  	ON DELETE RESTRICT ON UPDATE RESTRICT
)
ENGINE = InnoDB
DEFAULT CHARSET = LATIN1
...
ENGINE=InnoDB
COMMENT='Content data main table'
DEFAULT CHARSET = LATIN1
ROW_FORMAT=FIXED;
...
--  PARENTXPATH TEXT NOT NULL,
  ...
  FDOUBLE DOUBLE PRECISION(54,16),
...
ENGINE=InnoDB
COMMENT='Content data detail table'        
DEFAULT CHARSET = LATIN1
ROW_FORMAT=DYNAMIC;
...
CREATE TABLE FXS_LOCK (
  ...
  FOREIGN KEY FK_LOCK_ID_VER(LOCK_ID,LOCK_VER) REFERENCES FX_CONTENT(ID,VER)
        ON DELETE CASCADE ON UPDATE RESTRICT,
  UNIQUE KEY UK_LOCK_PK(LOCK_ID,LOCK_VER),
  UNIQUE KEY UK_LOCK_RES(LOCK_RESOURCE(255))
)
ENGINE = InnoDB
DEFAULT CHARSET=LATIN1
COMMENT = 'Locks';
...
-- index of search_id lookup since primary key index is too large
...
CREATE TABLE FXS_BRIEFCASE_DATA (
  ...
  METADATA VARCHAR(4096) CHARACTER SET UTF8 NULL,
...
ENGINE=InnoDB
COMMENT='Scriptmapping table for assignments'
DEFAULT CHARSET=LATIN1
ROW_FORMAT=COMPRESSED;
...
ENGINE=InnoDB
COMMENT='Scriptmapping table for types'
DEFAULT CHARSET=LATIN1
ROW_FORMAT=COMPRESSED;
...
CREATE TABLE FXS_MANDATORCONFIGURATION (
    MANDATOR_NAME VARCHAR(255) CHARACTER SET UTF8 NOT NULL,
	CPATH VARCHAR(255) NOT NULL,
	CKEY VARCHAR(255) CHARACTER SET UTF8 NOT NULL,
	CVALUE LONGTEXT CHARACTER SET UTF8,
    CLASSNAME VARCHAR(255) NULL,
...
COMMENT='Hierarchical Treestore (Edit)'
ENGINE=InnoDb;
...
COMMENT='Hierarchical Treestore (Live)'
ENGINE=InnoDb;
...
CREATE TABLE FX_PHRASE_VAL (
  ...
  SVAL TEXT CHARACTER SET UTF8 NOT NULL,
...
CREATE TABLE FX_PHRASE_MAP (
  ...
  PRIMARY KEY(MANDATOR, CAT, NODEID, NODEMANDATOR, PHRASEID, PMANDATOR),
  FOREIGN KEY (MANDATOR) REFERENCES FXS_MANDATOR(ID)
    	ON DELETE RESTRICT ON UPDATE RESTRICT,
...
ROW_FORMAT=FIXED;
CREATE INDEX FXI_PHRASE_MAP_KEY ON FX_PHRASE_MAP(PHRASEID, PMANDATOR, DIRECT);
CREATE INDEX FXI_PHRASE_MAP_POS ON FX_PHRASE_MAP(NODEMANDATOR, POS, DIRECT);
