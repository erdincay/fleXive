 ...
 * @since 3.1
 */
public class PostgresDialect extends GenericSqlDialect {
    private static final Log LOG = LogFactory.getLog(PostgresDialect.class);
    ...
    public PostgresDialect(FxEnvironment environment, ContentEngine contentEngine, CmisSqlQuery query, boolean returnPrimitives) {
            ...
            if (column.getSelectedObject() instanceof Selectable
                ...
                try {
                    if (LOG.isTraceEnabled()) {
                        LOG.trace("Multivalued properties with Postgres: SET group_concat_max_len := @@max_allowed_packet");
                    }
                    stmt = con.createStatement();
                    stmt.executeQuery("SET group_concat_max_len := @@max_allowed_packet");
                    break;
                } finally {
                    Database.closeObjects(PostgresDialect.class, null, stmt);
                }
    ...
    public ConditionMapper<ContainsCondition> conditionContain() {
        return PostgresContainsCondition.getInstance();
    }
    ...
    public ResultColumnMapper<ResultColumnReference> selectColumnReference() {
        return PostgresColumnReference.getInstance();
    }
