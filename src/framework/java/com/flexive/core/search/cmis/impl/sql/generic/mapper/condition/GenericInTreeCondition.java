...
import com.flexive.core.search.cmis.model.TreeCondition;
import com.flexive.core.storage.genericSQL.GenericTreeStorage;
import com.flexive.shared.tree.FxTreeMode;

/**
 * SQL mapper for IN_TREE conditions.
 * 
 * @author Daniel Lichtenberger (daniel.lichtenberger@flexive.com), UCS - unique computing solutions gmbh (http://www.ucs.at)
 * @version $Rev$
 */
public class GenericInTreeCondition implements ConditionMapper<TreeCondition> {
    private static final GenericInTreeCondition INSTANCE = new GenericInTreeCondition();

    public static GenericInTreeCondition getInstance() {
        return INSTANCE;
    }
    ...
    public String getConditionSql(SqlMapperFactory sqlMapperFactory, TreeCondition condition, SelectedTableVisitor joinedTables) {
        ...
        return "(SELECT DISTINCT " + joinedTables.getSelectForSingleTable(condition.getTableReference()) + " FROM "
                ...
                + "WHERE t.lft > " + condition.getFolderInfo().getLeft()
                + " AND t.rgt < " + condition.getFolderInfo().getRight()
