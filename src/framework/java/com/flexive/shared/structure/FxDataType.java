...
public enum FxDataType implements Serializable, SelectableObject {
    ...
    DateTime(9, true),
    DateRange(10, true),
    DateTimeRange(11, true),
    Boolean(12, true),
    Binary(13, true),
    Reference(14, true),
    InlineReference(15, false),
    SelectOne(16, true),
    SelectMany(17, true);
     ...
     */
    FxDataType(long id, boolean singleRowStorage) {
    ...
    public FxValue getRandomValue(Random rnd, FxPropertyAssignment assignment) {
        switch (this) {
            ...
            case Date:
                return new FxDate(assignment.isMultiLang(), new java.util.Date(rnd.nextLong()));
            case DateTime:
                return new FxDateTime(assignment.isMultiLang(), new java.util.Date(rnd.nextLong()));
            case DateRange:
                return new FxDateRange(assignment.isMultiLang(), new DateRange(new java.util.Date(rnd.nextLong()), new java.util.Date(rnd.nextLong())));
            case DateTimeRange:
                return new FxDateTimeRange(assignment.isMultiLang(), new DateRange(new java.util.Date(rnd.nextLong()), new java.util.Date(rnd.nextLong())));
            case Boolean:
            ...
            case SelectOne:
                //TODO: set a random item
                return new FxSelectOne(assignment.isMultiLang(), assignment.getProperty().getReferencedList().getItems().get(0));
            case SelectMany:
                //TODO: select random items
                return new FxSelectMany(assignment.isMultiLang(), new SelectMany(assignment.getProperty().getReferencedList())).setEmpty();
            case InlineReference:
            default:
