...
CREATE TABLE FXS_MANDATOR (
 ...
 CREATED_BY NUMERIC UNSIGNED NOT NULL,
 CREATED_AT DATETIME NOT NULL,
 MODIFIED_BY NUMERIC UNSIGNED NOT NULL,
...
CREATE TABLE FXS_ACCOUNTS (
 ...
 DEFAULT_NODE INTEGER UNSIGNED UNSIGNED,
...
CREATE TABLE FXS_ACL (
 ...
 CAT_TYPE NUMERIC NOT NULL COMMENT '1=Content, 2=Structure, 3=Workflow',
...
CREATE TABLE FXS_ROLEMAPPING (
  ACCOUNT INTEGER UNSIGNED NOT NULL,
  USERGROUP INTEGER UNSIGNED NOT NULL,
  ROLE  NUMERIC UNSIGNED NOT NULL,
...
CREATE TABLE FXS_TYPEDEF (
  ...
  WORKFLOW INTEGER UNSIGNED NOT NULL,
  MANDATOR INTEGER UNSIGNED NOT NULL COMMENT 'owning mandator',
  ...
  FOREIGN KEY FK_TYPEDEF_WORKFLOW(WORKFLOW) REFERENCES FXS_WORKFLOWS(ID)
  	ON DELETE RESTRICT ON UPDATE RESTRICT,
  FOREIGN KEY FK_TYPEDEF_MANDATOR(MANDATOR) REFERENCES FXS_MANDATOR(ID)
    ON DELETE RESTRICT ON UPDATE RESTRICT
)
ENGINE=InnoDB
DEFAULT CHARSET = LATIN1
...
CREATE TABLE FXS_TYPE2MANDATORS (
  TYPEID INTEGER UNSIGNED NOT NULL,
  MANDATORID INTEGER UNSIGNED NOT NULL,
  PRIMARY KEY (TYPEID, MANDATORID),
  FOREIGN KEY FK_TYPEDEFID(TYPEID) REFERENCES FXS_TYPEDEF(ID)
  	ON DELETE RESTRICT ON UPDATE RESTRICT,
  FOREIGN KEY FK_MANDATORID(MANDATORID) REFERENCES FXS_MANDATOR(ID)
    ON DELETE RESTRICT ON UPDATE RESTRICT
)
ENGINE=InnoDB
DEFAULT CHARSET = LATIN1
COMMENT='Type/Mandator mappings'
ROW_FORMAT=FIXED;
...
CREATE TABLE FXS_TYPEPROPS_T (
  ...
  LANG SMALLINT UNSIGNED NOT NULL,
  DESCRIPTION TEXT CHARACTER SET UTF8 COMMENT 'Description',
  DEFAULT_VALUE TEXT CHARACTER SET UTF8 COMMENT 'Language specific default value',
...
CREATE TABLE FXS_DIVISIONCONFIGURATION (
	...
	CPATH VARCHAR(255) NOT NULL,
	CKEY VARCHAR(255) NULL,
	CVALUE TEXT CHARACTER SET UTF8,
	PRIMARY KEY PK_DIVISIONCONFIGURATION(DIVISION_ID, CPATH, CKEY)
)
ENGINE = InnoDB
DEFAULT CHARSET = LATIN1
COMMENT = 'Per-division configuration table';
...
create table FXS_TEMPLATE (
  ...
  FINAL_CONTENT LONGTEXT CHARACTER SET UTF8,
  CREATED_BY NUMERIC UNSIGNED NOT NULL,
  CREATED_AT DATETIME NOT NULL,
  MODIFIED_BY NUMERIC UNSIGNED NOT NULL,
  MODIFIED_AT DATETIME NOT NULL,
  MASTER_TEMPLATE INTEGER UNSIGNED,
  ISLIVE BOOLEAN NOT NULL DEFAULT false,
  INSYNC BOOLEAN NOT NULL DEFAULT false,
  PRIMARY KEY(ID,ISLIVE),
  UNIQUE UK_TEMPLATE_NAME(NAME,ISLIVE),
  FOREIGN KEY FK_MASTERTEMPLATE(MASTER_TEMPLATE) REFERENCES FXS_TEMPLATE(ID)
  	ON DELETE RESTRICT ON UPDATE RESTRICT
)
ENGINE = InnoDB
DEFAULT CHARSET=LATIN1
...
create table FXS_TAG_RELATIONS (
 TAG_ID INTEGER UNSIGNED NOT NULL,
 TEMPLATE_ID INTEGER UNSIGNED NOT NULL,
 TEMPLATE_ISLIVE BOOLEAN NOT NULL,
 UNIQUE UK_TAG_RELATIONS(TAG_ID,TEMPLATE_ID),
 FOREIGN KEY FK_MASTERTEMPLATE(TAG_ID) REFERENCES FXS_TEMPLATE(ID)
  	ON DELETE RESTRICT ON UPDATE RESTRICT,
 FOREIGN KEY FK_MASTERTEMPLATE(TEMPLATE_ID,TEMPLATE_ISLIVE) REFERENCES FXS_TEMPLATE(ID,ISLIVE)
  	ON DELETE RESTRICT ON UPDATE RESTRICT
)
ENGINE = InnoDB
DEFAULT CHARSET=LATIN1
COMMENT = 'Holds all tags that are used in templates';
