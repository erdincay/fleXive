...
public class TreeNode extends UIOutput {
    ...
    public void writeTreeNodes(TreeNodeWriter writer) throws IOException {
        ...
        final String linkUrl = StringUtils.isNotBlank(link) ? FxJsfUtils.getRequest().getContextPath() + link : null;
        if (childNodes.size() == 0) {
            writer.writeNode(new Node(getId(), getTitle(), titleClass != null ? titleClass : Node.TITLE_CLASS_LEAF,
                    icon, linkUrl));
        } else {
            writer.startNode(new Node(getId(), getTitle(), titleClass != null ? titleClass : Node.TITLE_CLASS_NODE,
                    icon, linkUrl));
    ...
    public String getTitle() {
        if (title == null) {
            title = getStringValue(this, "title");
        }
        return title;
    }
    ...
    public String getTitleClass() {
        if (titleClass == null) {
            titleClass = getStringValue(this, "titleClass");
        }
        return titleClass;
    }
