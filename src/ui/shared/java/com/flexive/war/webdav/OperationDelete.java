...
/**
 * Delete operation for Non-Collection Resources and Collections.
 * <p/>
 * The DELETE method on a collection MUST act as if a "Depth: infinity" header was used on it.
 * A client MUST NOT submit a Depth header with a DELETE on a collection with any value but infinity.<br>
...
class OperationDelete extends Operation {
    ...
    public OperationDelete(HttpServletRequest req, HttpServletResponse resp, boolean readonly) {
        super(req, resp, readonly);
    }

    void writeResponse() throws IOException {
        int resultCode = FxWebDavServlet.getDavContext().deleteResource(request, path);
        if (resultCode != FxWebDavStatus.SC_OK)
            response.sendError(resultCode, "Failed to delete the resource");
    }
}
